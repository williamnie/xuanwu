...
2. **Agent 连接测试通过**：doctor 至少识别一个可用执行器，且当前浏览器会话内已有一次成功的 provider test。“打开连接向导”会进入普通 **Settings → Models & Agents**，在推荐 provider 卡片中执行现有的“测试连接并发现模型”。测试结果由现有 `provider_connection_tested` action event 审计，密钥不回显；向导只在 `sessionStorage` 保留 30 分钟的成功回执用于当前 UI checklist，它不是 provider 状态 authority，丢失后重新测试即可。
...
- **Agent 不可用**：先安装/登录 CLI，再在普通 Models & Agents 重新测试；不用 Advanced 的 base URL 表单做临时旁路。
