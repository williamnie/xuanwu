...
5. token 只能通过 `--token-file`、`CODEX_RUNNER_AUTH_TOKEN_FILE`、临时 curl config 或内存环境变量传递，不得输出实际 token。
...
```bash
./dist/codex-issue-runner issue update \
  --addr "${CODEX_RUNNER_ADDR:-127.0.0.1:3008}" \
  --token-file "$CODEX_RUNNER_AUTH_TOKEN_FILE" \
  --id <issue-id> \
  --status done \
  --json
```
