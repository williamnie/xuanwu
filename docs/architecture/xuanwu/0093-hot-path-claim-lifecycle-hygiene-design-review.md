...
- **§2.5 hygiene 缺口**属实：`release.yml` 无 hygiene 步骤；`package-release.sh:171-184` preflight 无 hygiene；`release-workflow.test.mjs` 只断言 ci.yml 含 7 个 gate；`node scripts/repository-hygiene-audit.mjs --json` 实测 ok=true（7/7 checks）。
