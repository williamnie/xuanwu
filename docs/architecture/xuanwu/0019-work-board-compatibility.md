...
- **Source of truth**：当前仍为 `issues`。Work Board 读取 `/api/works` 的 Issue 投影，create/edit 经 Work HTTP adapter 写回 Issues；不再投影 PI Action、Delegation 或 Watch 关系。
...
- `Attention=Needs attention` 只按可操作 Work 状态确定：`triage`、`pending_verification`、`failed`。
