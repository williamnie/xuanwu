...
- 签名 host binary、相邻 exact-pinned `xuanwu.qodercli.mjs` (`1.1.18`)、release/install/daemon tests
...
release 缺少相邻 `xuanwu.qodercli.mjs`；provider 不 ready；仓库或 state dir 不在上述隔离根目录；出现未知
...
"$QODER_ACCEPTANCE_INSTALL_DIR/xuanwu.qodercli.mjs" --version
test "$("$QODER_ACCEPTANCE_INSTALL_DIR/xuanwu.qodercli.mjs" --version | awk 'NR==1 {print $1}')" = '1.1.18'
```
...
- `xuanwu.qodercli.mjs` 始终存在且 executable；缺失 asset 的安装必须 fail closed；
