...
test('Guardian alert client and store expose open alerts slice with graceful fallback', () => {
  assert.match(assistantSource, /getPiGuardianAlerts:/);
  assert.match(assistantSource, /\/api\/pi\/guardian\/alerts/);
  assert.match(assistantSource, /ackPiGuardianAlert:/);
  assert.match(assistantSource, /\/api\/pi\/guardian\/alerts\/\$\{encodeURIComponent\(id\)\}\/ack/);
