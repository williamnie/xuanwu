...
export function configureGuide(connector) {
  ...
  if (id.endsWith('-events')) return { title: 'Git provider 配置', body: 'Git 事件连接复用现有 GitHub/GitLab provider credential 与 repository mapping，不在 Connections 创建第二份 token。', refs };
  ...
  return { title: 'Connector manifest 配置', body: '按当前 manifest/env 声明补齐缺失项，然后刷新 health；Connections 不读取 secret material。', refs };
}
