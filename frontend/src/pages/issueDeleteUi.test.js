...
test('issue delete UI uses DELETE API and protects in-progress issues', () => {
  assert.match(clientSource, /deleteIssue:\s*\(id\)\s*=>\s*request\(`\/api\/issues\/\$\{id\}`,\s*\{[\s\S]*method:\s*'DELETE'/);
  assert.match(cardSource, /const canDelete = issue\.status !== 'in_progress'/);
  assert.match(detailSource, /issue\.status !== 'in_progress'[\s\S]*<Trash2 size=\{14\} \/> 删除/);
  assert.match(issuesSource, /api\.deleteIssue\(issueToDelete\.id\)/);
