...
export type ImProjectMapping = { chatId?: string; projectId: string; userId?: string };
export type ImProjectContextStatus = "resolved" | "ambiguous" | "missing";
export type ImProjectContextSource =
  "issue_ref" | "explicit_project" | "user_switch" | "card_select" |
  "interaction_select" | "mapping_default" | "none";
export type ImProjectContextConfidence = "high" | "medium" | "low" | "none";
export type ImProjectContextIssue = { id: number; project_id: string };
export type ImProjectContextProject = { id: string; name?: string };
export type ImProjectContextActiveProject = { active_project_id?: string; active_project_source?: string };
export type ImProjectContextMessage = { chatId?: string; senderId?: string; senderOpenId?: string };
export type ImProjectContextInput = {
  activeProject?: ImProjectContextActiveProject | null;
  issues?: ImProjectContextIssue[];
  mappings?: ImProjectMapping[];
  message?: ImProjectContextMessage;
  projects?: ImProjectContextProject[];
  text: string;
};
export type ImProjectContextDatabaseInput =
  Omit<ImProjectContextInput, "activeProject" | "issues" | "projects"> & { scopeKey?: string };
export type ImProjectContextResult = {
  candidates: string[];
  confidence: ImProjectContextConfidence;
  projectId: string;
  reason: string;
  source: ImProjectContextSource;
  status: ImProjectContextStatus;
};

type ProjectMatch = { id: string; score: number };

export function resolveImProjectContext(input: ImProjectContextInput): ImProjectContextResult {
  ...
  return mapped.status !== "missing" ? mapped : missing("no_project_context");
}

export function resolveImProjectContextFromDatabase(
  db: RunnerDatabase,
  input: ImProjectContextDatabaseInput
): ImProjectContextResult {
  return resolveImProjectContext({
    ...input,
...
function issuesFromDatabase(db: RunnerDatabase, text: string): ImProjectContextIssue[] {
...
function issueProject(text: string, issues: ImProjectContextIssue[]): ImProjectContextIssue | undefined {
  const ids = issueRefs(text);
  if (ids.length === 0) return undefined;
  return ids
    .map((id) => issues.find((issue) => issue.id === id && clean(issue.project_id) !== ""))
    .find((issue): issue is ImProjectContextIssue => issue !== undefined);
}

function explicitProject(text: string, projects: ImProjectContextProject[]): ImProjectContextResult {
  const matches = bestProjectMatches(text, projects);
  if (matches.length === 0) return missing("no_explicit_project_text");
  if (matches.length === 1) return resolved(matches[0].id, "explicit_project", "high", "explicit_project_text");
...
function mappingMatches(mapping: ImProjectMapping, message: ImProjectContextMessage): boolean {
...
function bestProjectMatches(text: string, projects: ImProjectContextProject[]): ProjectMatch[] {
...
function projectMatches(project: ImProjectContextProject, normalizedText: string): ProjectMatch[] {
  return projectTokens(project)
    .filter((token) => token !== "" && normalizedText.includes(token))
    .map((token) => ({ id: project.id, score: token.length }));
}

function projectTokens(project: ImProjectContextProject): string[] {
...
function ambiguous(candidates: string[], source: ImProjectContextSource, reason: string): ImProjectContextResult {
  return { candidates, confidence: "low", projectId: "", reason, source, status: "ambiguous" };
}

function missing(reason: string): ImProjectContextResult {
