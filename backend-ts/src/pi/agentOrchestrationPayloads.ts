...
function writeBackInstruction(role: AgentRole, target: Issue | null): string {
  if (!target || role !== "reviewer") return "";
  return [
    "Review output contract:",
    "- Inspect the requested high-risk area and report concrete findings, commands, and workspace facts.",
    `- Do not write lifecycle state for parent Issue #${target.id}; PI alone decides whether the parent is done, continues, retries, needs_user, or failed.`,
    "- Reviewer output is context for PI, never an acceptance gate."
  ].join("\n");
}

function parentIssueContext(role: AgentRole, target: Issue): string {
  return `Parent issue: #${target.id} ${target.title}`;
}
