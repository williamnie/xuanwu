...
export function redactedProviderErrorSummary(texts: string[], statusCode?: number): string {
  const picked = texts.find((text) =>
    /error|429|rate|retry|capacity|overload|serveroverloaded|reconnecting|disconnect|unauthorized|permission|timeout|timed out|network|transport|deadline|eof/i.test(text));
