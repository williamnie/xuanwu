...
test("Guardian recovery action refreshes a stable issue snapshot after provider error annotation", async () => {
  ...
  try {
    ...
    const [action] = JSON.parse(candidate.actions_json) as Array<Record<string, unknown>>;
    const payload = action?.payload as Record<string, unknown>;

    expect(payload).toMatchObject({
      expected_issue_status: "in_progress",
      expected_issue_updated_at: "2026-07-18T06:21:17Z",
      expected_run_id: "issue-737-attempt-1",
