...
function issueActionTools(actions: PiRunnerActionLayer): ToolDefinition[] {
  return [
    ...
    actionTool("issue_completion_reconcile", "Issue Completion Reconcile",
      "Compatibility action that requests issue-scoped PI semantic acceptance for an ended Run. It does not classify commands or complete the Issue through the legacy Evidence gate.",
      Type.Object({ issue_id: positiveID, rationale: optionalString }, objectOptions),
      actions.reconcileIssueCompletion),
    actionTool("issue_read", "Issue Read", "Read one runner issue with full body, allowed status targets, dependency readiness, compact Run state, recent events, Completion Card, Evidence, and Handoff context.",
