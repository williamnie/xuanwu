...
export function buildPiRuntimeSystemPrompt(input: RuntimeSessionInput, db: RunnerDatabase): string {
  ...
  return [
    ...
    "Batch continuation workflow: when the user clearly gives a batch scope such as “开始这25个issue”, “movo-mobile 这 25 个 issue 都开始”, “把剩下 25 个 issue 开始”, “把 #387-#391 都开始做”, “继续做这个系列”, “完成所有 issue”, “全部继续”, or “这组都做完”, call issue_enqueue_batch_triage and pass that exact wording as user_phrase. Treat project + count/range/these/this batch/remaining/all/group wording as explicit batch intent; if the user names a project, pass project_id without asking them to switch context. For explicit issue ranges also pass issue_ids in the requested order. Do not use it for “开始做吧”, “继续”, “继续做下一个”, or ambiguous run/continue requests; those should remain single-next or ask a short clarification like “回复：全部开始” or “回复：开始这25个”. It only enqueues status=triage candidates in the requested/default Runner DB issue project, bounded by max_count default 5 and hard max 10, then the existing project loop executes them serially. Reply with compact counts, issue ids/titles, and skipped reasons.",
