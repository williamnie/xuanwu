...
function issueManagementWorkflow(): string {
  return [
    ...
    "For explicit Issue moves, issue_status_update only controls triage, todo, in_progress, or cancelled. It must never move needs_user to in_progress: read issue_read.decision and call human_review_response with the current request id/revision. accept on decision/risk_acceptance records the choice without a new execution Session so PI may continue; accept on acceptance closes delivery. Authorization, budget, credentials, installation details, or requested information are not delivery acceptance; request_changes continues the same Provider Session in a new Run/Turn. done, failed, and needs_user are semantic outcomes and must be written only from a PI decision.",
...
export function xuanwuSupervisorCompatibilityPrompt(): string {
  return [
    "Compatibility prompt (temporary adapter, not a second product model): use Work, Run, Workflow, Evidence, Handoff, Attention, and Automation for internal reasoning, but expose those terms or identifiers in user-facing prose only when they help tracking, audit, or disambiguation. Prefer the registered work_* and run_* domain tools. Read Evidence and Handoff through the current issue/run/repository projections; never invent evidence_* or handoff_* tool names unless capability_search actually returns them.",
...
function workToolWorkflow(): string {
  return [
    ...
    "Completion notifications: call issue_execution_status per target first. Terminal means report now and create no watch. Otherwise call issue_completion_watch_create only for unfinished targets and promise notification only after success. target_channel uses canonical connector ids (feishu/telegram), never runtime source names; the trusted current IM target wins.",
    ...
    "IM channels are transports, not persistent Project context. A project_id or issue_id is a one-turn target; later generic questions and runner-wide counts must not inherit it unless the current user turn states it again.",
...
function automaticReusableMemoryPolicy(): string {
  return [
    ...
    "Never store current or historical Work/Run/Issue status, counts, queue emptiness, timestamps, manager-cycle summaries, raw logs, or unverified guesses. For every current status question, query authoritative work/run/issue/repository tools even when related memory exists.",
