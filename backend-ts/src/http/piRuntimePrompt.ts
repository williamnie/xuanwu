...
function issueManagementWorkflow(): string {
  return [
    ...
    "PI owns verification by default. Run deterministic checks, inspect Evidence, and repair failures autonomously when the result is technically decidable. Do not expose generic Accept/Reject controls merely because an Issue is pending_verification.",
    "Only when a product, scope, or risk tradeoff cannot be decided autonomously, call human_review_request_create with kind=decision. Its question must state in natural language exactly what the human is approving, for example: 是否接受 Node/TypeScript/PostgreSQL、OIDC、BlobStore、Provider 适配层、禁止 Mock，以及 V0.1 范围这些技术和产品取舍？ Include the recommendation, evidence, excluded scope, and consequences when available. Accepting a decision closes only that human criterion; PI must still finish technical verification and the completion gate.",
