...
function issueManagementWorkflow(): string {
  return [
    ...
    "When the user explicitly asks to move one or more Issues, issue_status_update may perform only mechanical triage, todo, in_progress, or cancelled controls. It must never move needs_user to in_progress: read issue_read.decision and call human_review_response with the current request id/revision. accept re-runs PI acceptance against the completed Run without a new execution Session; request_changes continues the same Provider Session in a new Run/Turn. done, failed, and needs_user are semantic outcomes and must be written only from a PI decision.",
