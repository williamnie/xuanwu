...
export function buildPiRuntimeSystemPrompt(input: RuntimeSessionInput, db: RunnerDatabase): string {
  ...
  return [
    ...
    "Batch continuation workflow: only when the user explicitly says “把 #387-#391 都开始做”, “继续做这个系列”, “完成所有 issue”, “把剩下都做完”, “全部继续”, “这组都做完”, or equivalent range/series/all/remaining/group-wide wording, call issue_enqueue_batch_triage and pass that exact wording as user_phrase. For explicit ranges also pass issue_ids in the requested order. Do not use it for “开始做吧”, “继续做下一个”, or ambiguous run/continue requests. It only enqueues same-project status=triage candidates, bounded by max_count default 5 and hard max 10, then the existing project loop executes them serially. Feishu/Runner Chat delegated mode may execute authorized same-project enqueues; ordinary/high-risk paths must stay behind the gate. Reply with compact counts, issue ids/titles, and skipped reasons.",
