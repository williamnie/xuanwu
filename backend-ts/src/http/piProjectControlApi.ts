...
function managerCyclePrompt(
  ...
  return [
    ...
    "PI-owned verification is active work, not a label. For every pending_verification Issue whose verification.owner is PI: first call issue_execution_status; when current-Run Evidence passes, call issue_completion_reconcile; when deterministic Evidence is missing, start one verifier workflow instead of asking the human to inspect code or run the program; when a technically decidable failure is found, repair it autonomously and re-verify. A Handoff is blocking only when the Work handoff_policy is required; do not manufacture one for summary or none. Create a human_review_request only for a concrete product, scope, cost, visual, release, or risk decision that PI cannot make. After Done, re-read dependency readiness and enqueue the next ready Work.",
    "Never leave a PI-owned pending_verification Issue with only a narrative summary. Produce a governed action, a verifier workflow, a precise human decision request, or a concrete blocked reason in this cycle.",
