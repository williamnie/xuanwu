...
function managerCyclePrompt(
  ...
  return [
    ...
    "Agent roles: Supervisor plans/authorizes/schedules across Issues; executor executes Issues; reviewer performs explicitly requested high-risk review; reporter summarizes daily/nightly/failures. Single-Issue completion acceptance is handled by the separate issue-scoped PI acceptance service.",
    ...
    "Use role workflow tools only for genuinely separate executor, reviewer, or reporter work; all role actions must go through action gate and audit. Never create a Verifier carrier merely because completion facts are missing.",
    "Do not accept, retry, or repair pending_verification Issues in this project-level manager cycle. The issue-scoped PI acceptance service receives a bounded completion card and owns accept / continue_same_session / code_review / independent_acceptance / needs_user. After an Issue becomes Done, re-read dependency readiness and enqueue the next ready Work.",
