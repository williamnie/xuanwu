...
const COLUMNS = `id, project_id, title, status, intent_json, authorization_json,
  ...
  allowed_mcp_capabilities_json, allowed_skill_intents_json, audit_source, next_heartbeat_at, last_heartbeat_at,
  created_at, updated_at`;
const UPDATE_COLUMNS = [
  "project_id", "title", "status", "intent_json", "authorization_json",
  "scope_json", "starts_at", "expires_at", "allowed_actions_json",
  "forbidden_actions_json", "allowed_mcp_capabilities_json", "allowed_skill_intents_json", "audit_source",
...
export function createPiDelegation(db: RunnerDatabase, input: PiDelegationInput): PiDelegation {
  ...
  db.sqlite.run(`insert into ${TABLE} (${COLUMNS}) values (${placeholders(18)})`, [
    ...
    record.allowed_actions_json, record.forbidden_actions_json, record.allowed_mcp_capabilities_json,
    record.allowed_skill_intents_json, record.audit_source, record.next_heartbeat_at, record.last_heartbeat_at,
...
function normalizeDelegationCreate(input: PiDelegationInput): PiDelegation {
  ...
  return {
    ...
    allowed_mcp_capabilities_json: normalizeMcpCapabilityList(input.allowed_mcp_capabilities_json ?? auth.allowed_mcp_capabilities ?? auth.allowedMcpCapabilities),
...
function normalizeDelegationPatch(input: PiDelegationInput): PiDelegationInput {
  return {
    ...
    allowed_mcp_capabilities_json: input.allowed_mcp_capabilities_json === undefined ? undefined : normalizeMcpCapabilityList(input.allowed_mcp_capabilities_json),
